<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
   ...
   <xsl:template match="*" mode="inmeta">
      ...
      <xsl:for-each select="*">
         <xsl:choose>
            <xsl:when test="matches(name(),'idDocumento')">
               <identifier xtf:meta="true" xtf:tokenize="no">
                  <xsl:value-of select="string()"/>
	       </identifier>
               <idDocumento xtf:meta="true" xtf:tokenize="no">
                  <xsl:value-of select="string()"/>
	       </idDocumento>
            </xsl:when>
            <xsl:when test="matches(name(),'facet-tipoDocumento')">
               <facet-tipoDocumento xtf:meta="true" xtf:facet="yes" xtf:tokenize="no">
                  <xsl:copy-of select="@*"/>
                  <xsl:value-of select="string()"/>
               </facet-tipoDocumento>
            </xsl:when>
            <xsl:when test="matches(name(),'facet-autoridade')">
               <facet-autoridade xtf:meta="true" xtf:facet="yes" xtf:tokenize="no">
                  <xsl:copy-of select="@*"/>
                  <xsl:value-of select="string()"/>
               </facet-autoridade>
            </xsl:when>
            <xsl:when test="matches(name(),'facet-doutrinaAutor')">
               <facet-doutrinaAutor xtf:meta="true" xtf:facet="yes" xtf:tokenize="no">
                  <xsl:copy-of select="@*"/>
                  <xsl:value-of select="string()"/>
               </facet-doutrinaAutor>
            </xsl:when>
            <xsl:when test="matches(name(),'facet-doutrinaLingua')">
               <facet-doutrinaLingua xtf:meta="true" xtf:facet="yes" xtf:tokenize="no">
                  <xsl:copy-of select="@*"/>
                  <xsl:value-of select="string()"/>
               </facet-doutrinaLingua>
            </xsl:when>
            <xsl:when test="matches(name(),'facet-doutrinaClasse')">
               <facet-doutrinaClasse xtf:meta="true" xtf:facet="yes" xtf:tokenize="no">
                  <xsl:copy-of select="@*"/>
                  <xsl:value-of select="string()"/>
               </facet-doutrinaClasse>
            </xsl:when>
            <xsl:when test="matches(name(),'facet-doutrinaBibDigital')">
               <facet-doutrinaBibDigital xtf:meta="true" xtf:facet="yes" xtf:tokenize="no">
                  <xsl:copy-of select="@*"/>
                  <xsl:value-of select="string()"/>
               </facet-doutrinaBibDigital>
            </xsl:when>
            <xsl:when test="matches(name(),'facet-doutrinaBiblioteca')">
               <facet-doutrinaBiblioteca xtf:meta="true" xtf:facet="yes" xtf:tokenize="no">
                  <xsl:copy-of select="@*"/>
                  <xsl:value-of select="string()"/>
               </facet-doutrinaBiblioteca>
            </xsl:when>
            <xsl:when test="matches(name(),'facet-acronimo')">
               <facet-acronimo xtf:meta="true" xtf:facet="yes" xtf:tokenize="no">
                  <xsl:copy-of select="@*"/>
                  <xsl:value-of select="string()"/>
               </facet-acronimo>
            </xsl:when>
            <xsl:when test="matches(name(),'facet-localidade')">
               <facet-localidade xtf:meta="true" xtf:facet="yes" xtf:tokenize="no">
                  <xsl:copy-of select="@*"/>
                  <xsl:value-of select="string()"/>
               </facet-localidade>
            </xsl:when>
            <xsl:when test="matches(name(),'doutrinaReferenciada')">
            </xsl:when>
            <xsl:otherwise>
   ...
   <xsl:template match="*[matches(local-name(),'^date$')]" mode="facet">
      <facet-date>
         ...
         <xsl:choose>
		   ...
		   <xsl:variable name="decada" select="substring(., 1, 3)"/>
		   <xsl:value-of select="concat($decada, '0s::', replace(.,'-','::'))"/>
            </xsl:when>
            <xsl:when test="matches(.,'[0-9]{4}')">
           <xsl:variable name="decada" select="substring(., 1, 3)"/>
           <xsl:value-of select="concat($decada, '0s::', .)"/>
            </xsl:when>
         </xsl:choose>
      </facet-date>
   </xsl:template>
   ...
   <xsl:function name="parse:title">
      ...
      <xsl:choose>
         ...
         <xsl:when test="matches($parse-title, '^o ')">
            <xsl:value-of select="replace($parse-title, '^o (.+)', '$1')"/>
         </xsl:when>
         <xsl:when test="matches($parse-title, '^um ')">
            <xsl:value-of select="replace($parse-title, '^um (.+)', '$1')"/>
         </xsl:when>
         <xsl:when test="matches($parse-title, '^uma ')">
            <xsl:value-of select="replace($parse-title, '^uma (.+)', '$1')"/>
         </xsl:when>
         <xsl:otherwise>
   ...
   <xsl:template match="dc">
		 ...
		 <xsl:apply-templates select="/*" mode="inmeta"/>
