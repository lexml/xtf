...
public class SpellWriter 
{
  ...
  private final HashMap<String, Integer> recentWords = new HashMap<String, Integer>(
    MAX_RECENT_WORDS);
  ...
  private final HashMap<String, Integer> recentPairs = new HashMap<String, Integer>(
    MAX_RECENT_PAIRS);
  ...
  final Pattern splitPat = Pattern.compile("\\|");

  /** Memory limit for sorting */
  private final int SORT_MEM_LIMIT = 100 * 1024 * 1024; // 10 megs per file (2 files)

  /** Character array for forming combo keys */
  private final char[] keyChars = new char[4];

  /** String buffer for edmap pairs */
  private final StringBuffer edmapBuf = new StringBuffer();
    ...
    throws IOException 
  {
    ...
    edmapSorter.finish(new FileSorter.Output() 
      {
        String curKey = null;
        final ArrayList<String> curWords = new ArrayList<String>();
        int nWritten = 0;
