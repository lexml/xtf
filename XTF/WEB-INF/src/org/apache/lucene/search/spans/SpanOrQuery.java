...
public class SpanOrQuery extends SpanQuery 
{
  ...
  public SpanQuery[] getClauses() {
    return (SpanQuery[])clauses.toArray(new SpanQuery[0]);
  }
  ...
  public Query[] getSubQueries() {
    return (Query[])clauses.toArray(new Query[0]);
  }
  ...
  private static class SpanQueue extends PriorityQueue
  {
    ...
    throws IOException 
  {
    ...
    return new Spans() 
    {
      private final List all = new ArrayList(clauses.size());
      private final SpanQueue queue = new SpanQueue(clauses.size());
