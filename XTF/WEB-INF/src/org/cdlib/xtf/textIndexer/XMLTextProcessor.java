...
public class XMLTextProcessor extends DefaultHandler 
{
   ...
   */
  private BlockingQueue<FileQueueEntry> fileQueue = new ArrayBlockingQueue<>(1000000);
   ...
   *  description.
   */
  public static final String xtfUri = "http://cdlib.org/xtf";
  ...
  private static void configureIndexWriter(IndexWriter indexWriter) {
      indexWriter.setMaxBufferedDocs(90000);
      indexWriter.setMergeFactor(40);
   ...
   *
   */
  public void queueText(IndexSource srcInfo, boolean deleteFirst) throws InterruptedException {
    fileQueue.put(new FileQueueEntry(srcInfo, deleteFirst));
    ...
    throws IOException 
  {
    ...
    if (fileQueue.isEmpty() || !fileQueue.peek().deleteFirst)
      return;
  ...
  public void processQueuedTexts()
    ...
    final LuceneDocBuilderHandler handler = new LuceneDocBuilderHandler(indexInfo.stripWhitespace);
    ...
    while (true)
    {
      ...
      // Get the next file.
      FileQueueEntry ent = fileQueue.take();
      if(ent.idxSrc == null) break;
   ...
   *
   */
  private void processText(IndexSource file, LuceneDocBuilderHandler handler)
    throws SAXException
  {
   ...
   */
  private void parseText(final LuceneDocBuilderHandler handler) //curIdxSrc, indexInfo.passThroughAtttribs, curPrettyKey
  {
    try 
    {
